...
# # Need to load the libraries needed to each of the workers
...
# }
# 
# parallel::parLapply(X = pathListToProcess, fun = tester,cl = cluster)
# 
# # Close the cluster
...
# Keeps track of the paths with _allYears.csv files to process.
# Will be used in parallel lapply call below.
pathListToProcess <- vector(mode = "list")
  ...
  for(spatialDir in currSpatialLevelDirs){
    ...
    for(file in currFiles){
      if(substring(text = file,first = nchar(file) - 12, last = nchar(file)) == "_allYears.csv"){
        ...
        # Dummy call is below
        print(file)
        # Add the current path/file to the list to process.
        pathListToProcess <- append(x = pathListToProcess, values = file)
      }
    }
  }
}

createPlotsAndPlotData <- function(inputPath){

  print(paste0("Processing file: ", inputPath))
  ...
  split1 <- strsplit(x = inputPath, split = "_allYears.csv")[[1]]
  ...
  # ****************************
  # Only continue processing if the 3 output files for the given input file/path haven't already been created (prevents overwriting)
  # ****************************
  if(!file.exists(plotOutputFileNamePath) && !file.exists(plotly_lines_fileNamePath) && !file.exists(plotly_markers_fileNamePath)){
  
    currRotationResults <- read.table(file = inputPath, header = T, sep = ",")
    ...
    print(paste0("Skipping processing for file: ", inputPath, " because it's already been processed."))
  }
}

# parallel::parLapply(X = pathListToProcess, fun = createPlotsAndPlotData,cl = cluster)
# 
# # Close the cluster
# parallel::stopCluster(cluster)
